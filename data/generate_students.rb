...
def make_student(id)
  ...
  for i in 1..class_count
      ...
      this_class = { dow: dow, start_time: start_time, end_time: end_time, building: building_choice}
      classes << this_class
  end
...
(1..student_count).each do |n|
  ...
  student = make_student(n)
  last_student = (n == student_count)
  add_comma = last_student ? '' : ', '
  puts student.to_json + add_comma if $output
  $handle.puts student.to_json + add_comma
